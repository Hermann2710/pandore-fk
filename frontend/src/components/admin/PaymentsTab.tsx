...
import {
  useAdminPaymentMethods, useAdminPayments,
...
function MethodForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 rounded-xl border bg-card p-5">
      <h3 className="font-semibold">{initial ? "Edit" : "New"} Payment Method</h3>
      <div className="grid sm:grid-cols-2 gap-4">
        <div className="space-y-1.5">
          <Label>Name *</Label>
          <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. MTN Mobile Money" required />
        </div>
        <div className="space-y-1.5">
      ...
      <div className="space-y-1.5">
        <Label>Instructions (shown at checkout)</Label>
        <textarea
          value={instructions} onChange={(e) => setInstructions(e.target.value)}
          placeholder="e.g. Send payment to +237 6XX XXX XXX then enter your reference number."
...
export default function PaymentsTab() {
  ...
  const handleCreate = (fd: FormData) => create(fd, { onSuccess: () => setShowForm(false) });
  const handleUpdate = (fd: FormData) => {
    if (!editing) return;
    update({ id: editing.id, data: fd }, { onSuccess: () => setEditing(null) });
  };
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold flex items-center gap-2"><CreditCard className="h-5 w-5 text-primary" /> Payment Methods</h2>
          {!showForm && !editing && (
        ...
        <AnimatePresence>
          {showForm && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}>
              <MethodForm onSubmit={handleCreate} onCancel={() => setShowForm(false)} isPending={creating} />
            </motion.div>
          )}
        </AnimatePresence>
        ...
        {isLoading ? (
          ...
          <p className="text-muted-foreground text-sm py-6 text-center">No payment methods yet. Add one above.</p>
        ) : (
                ...
                <AnimatePresence>
                  {editing?.id === method.id ? (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                      <MethodForm initial={method} onSubmit={handleUpdate} onCancel={() => setEditing(null)} isPending={updating} />
                    </motion.div>
                  ) : (
                    <motion.div layout className="flex items-center gap-4 rounded-xl border bg-card p-4">
                      ...
                      <Badge variant={method.is_active ? "emerald" : "outline"}>{method.is_active ? "Active" : "Inactive"}</Badge>
          ...
          <div className="rounded-xl border overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-muted/50 text-muted-foreground">
                <tr>
                  {["Order", "Method", "Status", "Ref", "Date"].map((h) => (
                    ...
                    <tr key={p.id} className="border-t hover:bg-muted/30 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <span className={`flex items-center gap-1.5 ${statusColor[p.status]}`}>
                          <Icon className="h-4 w-4" />{p.status}
                        </span>
                      </td>
