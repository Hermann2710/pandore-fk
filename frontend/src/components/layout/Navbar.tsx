      ...
      <div className="mx-auto max-w-7xl px-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1">
            ...
            <span>{t("deliverTo")} <span className="text-white font-medium">{location}</span></span>
          </div>
          {currencies.length > 0 && (
            <select value={currency.code}
              onChange={(e) => { const found = currencies.find((c) => c.code === e.target.value); if (found) setCurrency(found); }}
        ...
        <div className="flex items-center gap-5">
          ...
          <Link href="/catalog" className="hover:text-white transition-colors">{t("newArrivals")}</Link>
          <Link href="/catalog?ordering=price" className="hover:text-white transition-colors">{t("bestSellers")}</Link>
          {threshold && <span className="text-emerald-400 font-medium">{t("freeShipping", { threshold })}</span>}
          <LanguageSwitcher />
        </div>
      </div>
    </div>
...
function SearchBar() {
  ...
  return (
    <form onSubmit={handleSearch} className="flex-1 max-w-2xl">
      <motion.div animate={{ scale: focused ? 1.01 : 1 }} transition={{ duration: 0.15 }}
            ...
            {!user ? (
              <div className="p-4 space-y-2">
                ...
                <p className="text-xs text-center text-slate-500">
                  {t("newCustomer")}{" "}
                  <Link href="/register" onClick={() => setOpen(false)} className="text-emerald-600 font-medium hover:underline">
                    {t("startHere")}
                  </Link>
                </p>
              </div>
              ...
              <div className="py-2">
                ...
                <div className="py-1">
                  <MenuItem href="/profile" icon={User} label={t("myProfile")} onClick={() => setOpen(false)} />
                  <MenuItem href="/orders" icon={User} label={t("myOrders")} onClick={() => setOpen(false)} />
                </div>
                <div className="border-t py-1">
                  {user.role === "customer" && (
                    <>
                      <MenuItem href="/wishlist" icon={Heart} label={t("myWishlist")} onClick={() => setOpen(false)} />
                      <MenuItem href="/cart" icon={ShoppingCart} label={t("myCart")} onClick={() => setOpen(false)} />
                    </>
                  )}
                  {user.role === "admin" && <MenuItem href="/admin" icon={LayoutDashboard} label={t("adminDashboard")} onClick={() => setOpen(false)} />}
                  {user.role === "delivery" && <MenuItem href="/delivery" icon={Truck} label={t("myQueue")} onClick={() => setOpen(false)} />}
                </div>
                <div className="border-t py-1">
...
function ActionIcons() {
  ...
  return (
    <div className="flex items-center gap-3 sm:gap-4">
      <Link href="/wishlist" className="relative text-white hover:text-rose-300 transition-colors">
      ...
      <Link href="/cart" className="flex items-end gap-1 text-white hover:text-emerald-300 transition-colors">
      ...
      <div className="mx-auto max-w-7xl px-4">
        <div className="hidden md:flex items-center gap-1 h-10">
              ...
              <AnimatePresence>
                {hoveredCat?.id === cat.id && (
                  <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 4 }}
                    transition={{ duration: 0.15 }} onMouseEnter={() => handleMouseEnter(cat)} onMouseLeave={handleMouseLeave}
                    ...
                    <div className="p-3 space-y-1">
                      {[
                        { href: `/catalog?category=${cat.slug}`, label: t("allCategory", { name: cat.name }) },
                        { href: `/catalog?category=${cat.slug}&ordering=-created_at`, label: tc("newest") },
                        { href: `/catalog?category=${cat.slug}&ordering=price`, label: tc("priceAsc") },
                        { href: `/catalog?category=${cat.slug}&ordering=-price`, label: tc("priceDesc") },
                      ].map((link) => (
                    ...
                    <div className="border-t px-4 py-2.5 bg-slate-50">
                      <Link href={`/catalog?category=${cat.slug}`} className="text-xs text-emerald-600 font-semibold hover:underline">
                        {t("seeAllCategory", { name: cat.name })}
                      </Link>
                    </div>
                  </motion.div>
                )}
          ...
          <div className="ml-auto flex items-center gap-1">
            <Link href="/catalog?tags=new-arrival" className="px-3 h-full flex items-center text-xs text-emerald-400 font-semibold hover:text-emerald-300 transition-colors">{t("newArrivals")}</Link>
            <Link href="/catalog?tags=bestseller" className="px-3 h-full flex items-center text-xs text-amber-400 font-semibold hover:text-amber-300 transition-colors">{t("bestSellers")}</Link>
            <Link href="/catalog?tags=limited-edition" className="px-3 h-full flex items-center text-xs text-rose-400 font-semibold hover:text-rose-300 transition-colors">{tcommon("limitedEdition")}</Link>
          </div>
        </div>
        <div className="md:hidden">
          <button onClick={() => setMobileOpen((v) => !v)}
            className="flex items-center justify-between w-full py-2.5 text-sm text-white font-medium">
            <span className="flex items-center gap-2"><Menu className="h-4 w-4" /> {t("browseCategories")}</span>
            <motion.div animate={{ rotate: mobileOpen ? 180 : 0 }}><ChevronDown className="h-4 w-4" /></motion.div>
          </button>
          <AnimatePresence>
            {mobileOpen && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }}
                exit={{ height: 0, opacity: 0 }} className="overflow-hidden border-t border-slate-700">
                <div className="py-2 grid grid-cols-2 gap-1">
                  {categories?.map((cat) => (
                    <Link key={cat.id} href={`/catalog?category=${cat.slug}`} onClick={() => setMobileOpen(false)}
...
export default function Navbar() {
  ...
  return (
    <motion.div initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }}
      transition={{ duration: 0.4, ease: "easeOut" }} className="sticky top-0 z-40 w-full">
      <TopBar />
