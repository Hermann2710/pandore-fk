...
import {
  ...
  Search, ChevronDown, User, MapPin, Menu, Heart,
  Shield, Tag, FolderOpen, ClipboardList,
} from "lucide-react";
    ...
    <div className="bg-slate-900 text-slate-300 text-xs py-1.5 hidden md:block">
      <div className="mx-auto max-w-7xl px-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-5">
          ...
          <span className="text-emerald-400 font-medium">Free shipping over 50 000 FCFA</span>
        </div>
      </div>
    </div>
  );
...
function SearchBar() {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (query.trim()) { router.push(`/catalog?search=${encodeURIComponent(query.trim())}`); setQuery(""); }
  };
  ...
  return (
    <form onSubmit={handleSearch} className="flex-1 max-w-2xl">
      ...
      >
        ...
        />
        <motion.button whileTap={{ scale: 0.95 }} type="submit" className="bg-emerald-500 hover:bg-emerald-600 transition-colors px-5 flex items-center justify-center">
...
function AccountMenu() {
  ...
  return (
    <div ref={ref} className="relative">
      <button onClick={() => setOpen((v) => !v)} className="flex flex-col items-start text-white hover:text-emerald-300 transition-colors group">
          ...
          >
            {!user ? (
              <div className="p-4 space-y-2">
                <Link href="/login" onClick={() => setOpen(false)} className="block w-full text-center bg-emerald-500 hover:bg-emerald-600 text-white font-semibold py-2 rounded-lg text-sm transition-colors">
                  Sign In
                </Link>
                <p className="text-xs text-center text-slate-500">
                  New customer?{" "}
                  <Link href="/register" onClick={() => setOpen(false)} className="text-emerald-600 font-medium hover:underline">Start here</Link>
                </p>
              </div>
            ) : (
              <div className="py-2">
                ...
                <div className="px-4 py-2.5 border-b flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <span className="text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-emerald-100 text-emerald-700 capitalize">{user.role}</span>
                  </div>
                </div>
                ...
                <div className="border-t py-1">
                  {user.role === "customer" && (
                    <>
                      <MenuItem href="/orders" icon={ClipboardList} label="My Orders" onClick={() => setOpen(false)} />
                      <MenuItem href="/wishlist" icon={Heart} label="My Wishlist" onClick={() => setOpen(false)} />
                ...
                <div className="border-t py-1">
                  <button onClick={() => { logout(); setOpen(false); }} className="w-full flex items-center gap-3 px-4 py-2 text-sm text-red-600 hover:bg-red-50 transition-colors">
...
function CategoryStrip() {
  ...
  const handleMouseEnter = (cat: Category) => { if (timeoutRef.current) clearTimeout(timeoutRef.current); setHoveredCat(cat); };
  const handleMouseLeave = () => { timeoutRef.current = setTimeout(() => setHoveredCat(null), 150); };

  return (
    <div className="bg-slate-800 border-t border-slate-700">
      <div className="mx-auto max-w-7xl px-4">
        <div className="hidden md:flex items-center gap-1 h-10">
          <Link href="/catalog" className="flex items-center gap-2 px-3 h-full text-sm font-semibold text-white hover:bg-slate-700 transition-colors rounded-sm">
            <Menu className="h-4 w-4" /> All
          </Link>
          <div className="w-px h-5 bg-slate-600 mx-1" />

          {categories?.map((cat) => (
            <div key={cat.id} onMouseEnter={() => handleMouseEnter(cat)} onMouseLeave={handleMouseLeave} className="relative h-full">
              <Link href={`/catalog?category=${cat.slug}`}
                ...
                <FolderOpen className="h-3.5 w-3.5 text-emerald-400" />{cat.name}
              </Link>
              <AnimatePresence>
                {hoveredCat?.id === cat.id && (
                  <motion.div
                    initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 4 }} transition={{ duration: 0.15 }}
                    onMouseEnter={() => handleMouseEnter(cat)} onMouseLeave={handleMouseLeave}
                  ...
                  >
                    <div className="bg-emerald-500 px-4 py-3">
                      ...
                      {cat.description && <p className="text-emerald-100 text-xs mt-0.5">{cat.description}</p>}
                    </div>
                    <div className="p-3 space-y-1">
                      {[
                        { href: `/catalog?category=${cat.slug}`, label: `All ${cat.name}` },
                        { href: `/catalog?category=${cat.slug}&ordering=-created_at`, label: "New Arrivals" },
                        { href: `/catalog?category=${cat.slug}&ordering=price`, label: "Price: Low to High" },
                        { href: `/catalog?category=${cat.slug}&ordering=-price`, label: "Price: High to Low" },
                      ].map((link) => (
                        <Link key={link.href} href={link.href} className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-slate-600 hover:bg-emerald-50 hover:text-emerald-700 transition-colors">
                          <Tag className="h-3.5 w-3.5 text-slate-400" />{link.label}
                        </Link>
                      ))}
                    </div>
                    <div className="border-t px-4 py-2.5 bg-slate-50">
                      <Link href={`/catalog?category=${cat.slug}`} className="text-xs text-emerald-600 font-semibold hover:underline">
                        See all in {cat.name} →
                      </Link>
                    </div>
                  </motion.div>
          ...
          <div className="ml-auto flex items-center gap-1">
            <Link href="/catalog?tags=new-arrival" className="px-3 h-full flex items-center text-xs text-emerald-400 font-semibold hover:text-emerald-300 transition-colors">New Arrivals</Link>
            <Link href="/catalog?tags=bestseller" className="px-3 h-full flex items-center text-xs text-amber-400 font-semibold hover:text-amber-300 transition-colors">Bestsellers</Link>
            <Link href="/catalog?tags=limited-edition" className="px-3 h-full flex items-center text-xs text-rose-400 font-semibold hover:text-rose-300 transition-colors">Limited Edition</Link>
          </div>
        </div>

        {/* Mobile */}
        <div className="md:hidden">
          <button onClick={() => setMobileOpen((v) => !v)} className="flex items-center justify-between w-full py-2.5 text-sm text-white font-medium">
            <span className="flex items-center gap-2"><Menu className="h-4 w-4" /> Browse Categories</span>
            <motion.div animate={{ rotate: mobileOpen ? 180 : 0 }}><ChevronDown className="h-4 w-4" /></motion.div>
          </button>
          <AnimatePresence>
            {mobileOpen && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden border-t border-slate-700">
                <div className="py-2 grid grid-cols-2 gap-1">
                  {categories?.map((cat) => (
                    <Link key={cat.id} href={`/catalog?category=${cat.slug}`} onClick={() => setMobileOpen(false)} className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-slate-200 hover:bg-slate-700 transition-colors">
                      <FolderOpen className="h-3.5 w-3.5 text-emerald-400 shrink-0" />{cat.name}
                    </Link>
                  ))}
                </div>
              </motion.div>
        ...
        <div className="mx-auto max-w-7xl px-4 flex items-center gap-4 sm:gap-6">
          <Link href="/" className="flex items-center gap-2 shrink-0 group">
            ...
            <div className="hidden sm:block">
              <span className="text-xl font-black tracking-tight text-white">PAN<span className="text-emerald-400">DORE</span></span>
