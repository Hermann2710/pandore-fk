...
const NAV_ITEMS = [
  ...
  { label: "Active",     icon: Truck,           href: "/delivery/active" },
  { label: "Completed",  icon: CheckCircle,     href: "/delivery/completed" },
] as const;
  ...
  return (
    ...
    >
        ...
        <AnimatePresence>
          {!collapsed && (
            <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }}>
              ...
              <p className="text-[10px] text-slate-400 -mt-0.5 tracking-widest uppercase">Delivery</p>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
      ...
      <nav className="flex-1 py-4 space-y-1 px-2">
        {!collapsed && (
          <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-widest px-2 mb-3">My Portal</p>
        )}
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              ...
              {isActive && !collapsed && <motion.span layoutId="delivery-dot" className="ml-auto h-1.5 w-1.5 rounded-full bg-white" />}
            </Link>
          );
        })}
      </nav>
      ...
      <div className="px-2 pb-4 border-t border-slate-700 pt-3">
        <Link href="/" title={collapsed ? "Back to Store" : undefined} className="flex items-center gap-3 rounded-lg px-2 py-2.5 text-sm text-slate-400 hover:bg-slate-800 hover:text-white transition-all">
