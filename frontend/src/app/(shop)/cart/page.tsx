...
export default function CartPage() {
  ...
  const handleCheckout = () => {
    ...
    checkout({ shipping_address: address, items: items.map((i) => ({ product_id: i.product.id, quantity: i.quantity })) });
  };
                ...
                <Card>
                  <CardContent className="flex items-center gap-4 p-4">
                    <div className="relative h-20 w-20 rounded-lg overflow-hidden bg-muted shrink-0">
                      {item.product.image ? <Image src={item.product.image} alt={item.product.name} fill className="object-cover" /> : <div className="h-full w-full bg-muted" />}
                    </div>
          ...
          <Card className="sticky top-24">
            <CardContent className="p-6 space-y-6">
              ...
              <div className="space-y-2">
                <Label htmlFor="address" className="flex items-center gap-1.5"><MapPin className="h-4 w-4 text-primary" /> Shipping Address</Label>
                <Input id="address" placeholder="123 Luxury Ave, Paris…" value={address} onChange={(e) => setAddress(e.target.value)} />
              </div>
